...
function SayfaAc(Sayfa, AcilmaSekli) {
    if (AppModel.ClientLisansKontrol() == true) {
        ...
        if (Sayfa != '#UrunLisansKontrol') {
            SayfaAc("#UrunLisansKontrol", "none");
        }
        else {
            $.mobile.changePage($(Sayfa), { dataUrl: "/", transition: AcilmaSekli, changeHash: true });
        }
...
var AppModel = {
    Uygulama: {
        ...
        FooterBilgi: ko.observable('Dehanın 10\'da 1\'i yetenek 10\'da 9\'u da çalışmaktır.'),
    ...
    Ara: function (Txt)
    {
        ...
        if (AppModel.Dil == 1)
        {
            console.log(db.Kelimeler);
            sonuc = _.filter(db.Kelimeler, function (obj) {
                if (obj.Turkce.toLowerCase().startsWith(Txt))
                {
                    return obj;
                }
            });
        ...
        {
            sonuc = _.filter(db.Kelimeler, function (obj) {
                if (obj.Ingilizce.toLowerCase().startsWith(Txt)) {
                    return obj;
                }
            });
        }
        ...
        {
            sonuc = _.filter(db.Kelimeler, function (obj) {
                if (obj.Fransizca.toLowerCase().startsWith(Txt)) {
                    return obj;
                }
            });
        }
        ...
        var sonucIlk;
        sonucIlk =
            _(AppModel.Sonuclar()).chain().first(1).value();
        AppModel.SecilenSonuclar.removeAll()
        AppModel.SecilenSonuclar.pushAll(sonucIlk);
...
$(document).ready(function () {
    ko.applyBindings(AppModel, document.getElementById("#UrunLisansKontrol"));
    var DegerW = $sutunTxtAra.parent().width() - 30;
